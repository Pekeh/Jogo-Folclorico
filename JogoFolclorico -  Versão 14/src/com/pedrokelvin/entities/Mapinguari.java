...
public class Mapinguari extends Entity{
	...
	public static boolean isDeadMapinguari=false;
	private boolean isDamaged = false;
	private int damageFrames = 5, damageCurrent = 0;
	
	
	
	public Mapinguari(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, null);
		sprites = new BufferedImage[4];
		sprites[0] = Game.spritesheet.getSprite(0,  640, 32, 32);
		sprites[1] = Game.spritesheet.getSprite(32, 640, 32, 32);
		sprites[2] = Game.spritesheet.getSprite(64, 640, 32, 32);
		sprites[3] = Game.spritesheet.getSprite(96, 640, 32, 32);
	}
	...
	public boolean isColidding(int xnext, int ynext) {
		...
		for(int i=0; i<Game.mapinguari.size(); i++) {
			Mapinguari e = Game.mapinguari.get(i);
			if(e == this)
				continue;
	...
	public void render (Graphics g) {
		if(!isDamaged)
			...
			g.drawImage(MAPINGUARI_FEEDBACK, this.getX() - Camera.x, this.getY()- Camera.y, null);
		}
	}
