...
public class World {
			...
			for(int xx = 0; xx<map.getWidth(); xx++) {
				for(int yy = 0; yy<map.getWidth(); yy++) {
					...
					if(pixelAtual == 0xFF000000) {
						...
						//Trilho
						tiles[xx + (yy*WIDTH)] = new CenarioTile(xx*32,yy*32, Tile.TILE_TRILHO3);
					}
						...
						//Passagem
						if(Game.CUR_LEVEL==1 || Game.CUR_LEVEL==2)
							tiles[xx + (yy*WIDTH)] = new PassTile(xx*32,yy*32, Tile.FLORZINHA_TILE);
						else if(Game.CUR_LEVEL==3 || Game.CUR_LEVEL==4)
							tiles[xx + (yy*WIDTH)] = new PassTile(xx*32,yy*32, Tile.FLORZINHA_TILE);
						else if(Game.CUR_LEVEL==5)
							tiles[xx + (yy*WIDTH)] = new PassTile(xx*32,yy*32, Tile.FLORZINHA_TILE);
	...
	public void render (Graphics g) {
		...
		if(Game.enemies.size()!=0){
			...
			if(Game.CUR_LEVEL==1 || Game.CUR_LEVEL==2)
				tiles[passageX + (passageY*WIDTH)] = new PassTile(passX,passY, Tile.FLORZINHA_TILE);
			else if(Game.CUR_LEVEL==3 || Game.CUR_LEVEL==4)
				tiles[passageX + (passageY*WIDTH)] = new PassTile(passX,passY, Tile.FLORZINHA_TILE);
			else if(Game.CUR_LEVEL==5)
				tiles[passageX + (passageY*WIDTH)] = new PassTile(passX,passY, Tile.FLORZINHA_TILE);
