...
public class Capelobo extends Entity{
	...
	public static boolean isDeadCapelobo=false;
	private boolean isDamaged = false;
	private int damageFrames = 5, damageCurrent = 0;
	
	
	
	public Capelobo(int x, int y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, null);
		sprites = new BufferedImage[4];
		sprites[0] = Game.spritesheet.getSprite(0,  800, 32, 32);
		sprites[1] = Game.spritesheet.getSprite(32, 800, 32, 32);
		sprites[2] = Game.spritesheet.getSprite(64, 800, 32, 32);
		sprites[3] = Game.spritesheet.getSprite(96, 800, 32, 32);
	}
	...
	public boolean isColidding(int xnext, int ynext) {
		...
		for(int i=0; i<Game.capelobo.size(); i++) {
			Capelobo e = Game.capelobo.get(i);
			if(e == this)
				continue;
	...
	public void render (Graphics g) {
		if(!isDamaged)
			...
			g.drawImage(CAPELOBO_FEEDBACK, this.getX() - Camera.x, this.getY()- Camera.y, null);
		}
	}
